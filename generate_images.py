...
def create_random_crops(image_filename, num_crops, out_dir):
  ...
  for i in range(0, num_crops):
    ...
    x = random.randint(0,im_width - 28)
    y = random.randint(0,im_height - 28)
    outfile = uuid.uuid4().hex + '.jpg'
    command = "magick convert "+ image_filename + " -crop 28x28"+"+"+str(x)+"+"+str(y)+" " + os.path.join(out_dir, outfile)
    os.system(str(command))
    print("croped "+ image_filename)
...
for f in light_backgrounds: 
  create_random_crops(f, 10, light_backgrounds_crops_dir)

for f in dark_backgrounds: 
  create_random_crops(f, 10, dark_backgrounds_crops_dir)
